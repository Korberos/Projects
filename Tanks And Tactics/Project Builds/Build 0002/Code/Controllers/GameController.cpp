...
bool CGameController::Initialize(void)
{
	...
	//  Define the window. If the window definition fails, return false.
	if (!m_Window.DefineWindow(WINDOW_WIDTH, WINDOW_HEIGHT, 32, false, "Tanks and Tactics - Tech Demo (build 001)")) return false;
...
void CGameController::Update(void)
{
	...
	m_fTimeSlice = std::max(std::min((float)(m_nElapsedTime[0] - m_nElapsedTime[1]) / 1000.0f, 0.1f), 1.0f);
	m_nElapsedTime[1] = m_nElapsedTime[0];

	// Update the current GameState
	m_pGameState->Update(m_fTimeSlice);
}
...
void CGameController::Render(void)
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluPerspective(45.0,(GLdouble)m_Window.GetHeight() / (GLdouble)m_Window.GetWidth(), 0.1, 100.0);
