...
bool CState_Battle::Initialize(float* fpTimeSlice)
{
	m_pTimeSlice = fpTimeSlice;

	// Gather the necessart controllers
	...
	m_nMapTexture = m_pTexture->LoadTexture("./Assets/TestImage.png");
	...
	for (unsigned int i = 0; i < UNIT_COUNT; ++i)
	{
		...
		m_UnitList[i].SetPosition(RandFloat(0.0f, 2.0f), RandFloat(0.0f, 2.0f));
	}
	m_nSelectedUnit = -1;

	// Debugging
	m_nRaySphereCollision = 0;
...
int CState_Battle::Update(float ElapsedTime)
{
	if		(m_nCurrentBattlePhase == EFFECT)	return EffectPhaseUpdate(ElapsedTime);
	else if	(m_nCurrentBattlePhase == BATTLE)	return BattlePhaseUpdate(ElapsedTime);
	else if	(m_nCurrentBattlePhase == ACTION)	return ActionPhaseUpdate(ElapsedTime);
	return 0;
}
...
void CState_Battle::Render3D(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, m_pTexture->GetTexture(m_nMapTexture));
	...
	if (m_nSelectedUnit != -1)
	{
		...
		vec3f Unit		= m_UnitList[m_nSelectedUnit].GetPosition();
		...
		glBegin(GL_QUADS);
			glTexCoord2f(1.0f, 0.0f);		glVertex3f(Unit.x - Right.x + Up.x * 3.0f, Unit.y + Up.y * 3.0f, Unit.z - Right.z + Up.z * 3.0f);
			glTexCoord2f(0.0f, 0.0f);		glVertex3f(Unit.x + Right.x + Up.x * 3.0f, Unit.y + Up.y * 3.0f, Unit.z + Right.z + Up.z * 3.0f);
			glTexCoord2f(0.0f, 1.0f);		glVertex3f(Unit.x + Right.x + Up.x * 1.0f, Unit.y + Up.y * 1.0f, Unit.z + Right.z + Up.z * 1.0f);
			glTexCoord2f(1.0f, 1.0f);		glVertex3f(Unit.x - Right.x + Up.x * 1.0f, Unit.y + Up.y * 1.0f, Unit.z - Right.z + Up.z * 1.0f);
		glEnd();
		...
		if (m_nCurrentUnitPhase == ACTION_MENU)
		{
			...
			glBegin(GL_QUADS);
				glTexCoord2f(1.0f, 0.0f);		glVertex3f(Unit.x + Right.x * 1.0f + Up.x * 2.0f, Unit.y + Up.y * 2.0f, Unit.z + Right.z * 1.0f + Up.z * 2.0f);
				glTexCoord2f(0.0f, 0.0f);		glVertex3f(Unit.x + Right.x * 2.0f + Up.x * 2.0f, Unit.y + Up.y * 2.0f, Unit.z + Right.z * 2.0f + Up.z * 2.0f);
				glTexCoord2f(0.0f, 1.0f);		glVertex3f(Unit.x + Right.x * 2.0f + Up.x * 0.0f, Unit.y + Up.y * 0.0f, Unit.z + Right.z * 2.0f + Up.z * 0.0f);
				glTexCoord2f(1.0f, 1.0f);		glVertex3f(Unit.x + Right.x * 1.0f + Up.x * 0.0f, Unit.y + Up.y * 0.0f, Unit.z + Right.z * 1.0f + Up.z * 0.0f);
			glEnd();

			glBindTexture(GL_TEXTURE_2D, m_pTexture->GetTexture(m_nMenuChoice[1]));
			glBegin(GL_QUADS);
				glTexCoord2f(1.0f, 0.0f);		glVertex3f(Unit.x - Right.x * 0.5f + Up.x * 5.0f, Unit.y + Up.y * 5.0f, Unit.z - Right.z * 0.5f + Up.z * 5.0f);
				glTexCoord2f(0.0f, 0.0f);		glVertex3f(Unit.x + Right.x * 0.5f + Up.x * 5.0f, Unit.y + Up.y * 5.0f, Unit.z + Right.z * 0.5f + Up.z * 5.0f);
				glTexCoord2f(0.0f, 1.0f);		glVertex3f(Unit.x + Right.x * 0.5f + Up.x * 3.0f, Unit.y + Up.y * 3.0f, Unit.z + Right.z * 0.5f + Up.z * 3.0f);
				glTexCoord2f(1.0f, 1.0f);		glVertex3f(Unit.x - Right.x * 0.5f + Up.x * 3.0f, Unit.y + Up.y * 3.0f, Unit.z - Right.z * 0.5f + Up.z * 3.0f);
			glEnd();

			glBindTexture(GL_TEXTURE_2D, m_pTexture->GetTexture(m_nMenuChoice[2]));
			glBegin(GL_QUADS);
				glTexCoord2f(1.0f, 0.0f);		glVertex3f(Unit.x - Right.x * 2.0f + Up.x * 2.0f, Unit.y + Up.y * 2.0f, Unit.z - Right.z * 2.0f + Up.z * 2.0f);
				glTexCoord2f(0.0f, 0.0f);		glVertex3f(Unit.x - Right.x * 1.0f + Up.x * 2.0f, Unit.y + Up.y * 2.0f, Unit.z - Right.z * 1.0f + Up.z * 2.0f);
				glTexCoord2f(0.0f, 1.0f);		glVertex3f(Unit.x - Right.x * 1.0f + Up.x * 0.0f, Unit.y + Up.y * 0.0f, Unit.z - Right.z * 1.0f + Up.z * 0.0f);
				glTexCoord2f(1.0f, 1.0f);		glVertex3f(Unit.x - Right.x * 2.0f + Up.x * 0.0f, Unit.y + Up.y * 0.0f, Unit.z - Right.z * 2.0f + Up.z * 0.0f);
			glEnd();
...
void CState_Battle::Render2D(void)
{
	...
	if (m_bDebugMode == true)
	{
		...
		sprintf_s(DebugString, 64, "Mouse Relative: (%f, %f)", m_pInput->GetMouseX() / (float)WINDOW_WIDTH, m_pInput->GetMouseY() / (float)WINDOW_HEIGHT);
		...
		sprintf_s(DebugString, 64, "Ray Test Check: Button #%d", m_nRaySphereCollision);
		m_pFont->RenderText(m_nTestFont, 10, 120, DebugString);
	}
	...
	if (m_pInput->GetMouseButton(0) == 1)
	{
		...
		else
		{
			...
			if (m_nCurrentUnitPhase == ACTION_MENU)
			{
				...
				SpherePos = vec3f(Unit.x + Right.x * 1.5f + Up.x, Unit.y + Up.y, Unit.z + Right.z * 1.5f + Up.z);
				if (LineToSphereCollision(Cam, Ray, SpherePos, Up.magnitude()))
				{
					m_nRaySphereCollision = 1;
					m_nCurrentUnitPhase = MOVE_CHECK;
				}

				SpherePos = vec3f(Unit.x + Up.x * 4.0f, Unit.y + Up.y * 4.0f, Unit.z + Up.z * 4.0f);
				if (LineToSphereCollision(Cam, Ray, SpherePos, Up.magnitude()))
				{
					m_nRaySphereCollision = 2;
			...
			else if (m_nCurrentUnitPhase == MOVE_CHECK)
			{
				...
				if (m_pMap->RayCollision(Cam, Cam + Ray, MoveTo))
					m_UnitList[m_nSelectedUnit].SetMovePosition(MoveTo);
			}
...
int CState_Battle::EffectPhaseUpdate(float ElapsedTime)
{
...
int CState_Battle::BattlePhaseUpdate(float ElapsedTime)
{
...
int CState_Battle::ActionPhaseUpdate(float ElapsedTime)
{
	//  Move to the next BattlePhase
	m_nCurrentBattlePhase++;
	if (m_nCurrentBattlePhase == BATTLEPHASE_COUNT) m_nCurrentBattlePhase = 0;
