...
{
	DrawList_LowerGrid(FindMoves(Char));
}
...
void CMapController::DrawList_LowerGrid(std::vector<MovementArea> AreaList)
{
	// Render the boxes, given the positions that the unit can move to
	glNewList(m_nDrawList[LOWERGRID], GL_COMPILE);
		glBegin(GL_LINES);
			for (unsigned int k = 0; k < AreaList.size(); ++k)
			{
				unsigned int i = AreaList[k].Row;
				unsigned int j = AreaList[k].Col;
...
std::vector<MovementArea> CMapController::FindMoves(CharacterUnit* Char)
{
	// If no character is passed in, return an empty vector
	if (Char == 0) return (std::vector<MovementArea>());

	// Create all required objects
	std::vector<MovementArea>	AreaList;
	...
	AreaList.push_back(StartPosition);
	...
	// Return the completed path list
	return AreaList;
}