...
private:
	...
	void DrawList_LowerGrid(std::vector<MovementArea> Paths);	// Creates a draw list of the blocks making up the lower grid
	void DrawList_CulledTriangles(vec3f vBegin, vec3f vEnd);	// Creates a draw list of the culled triangles (when aiming)

	std::vector<MovementArea>	FindMoves(CharacterUnit* Char);	// Creates a list of possible paths and returns the list
};
