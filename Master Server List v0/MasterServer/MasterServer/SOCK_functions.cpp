...
void Winsock_Exit()
{
	...
	for(int i = 0; i < buffers.count; i++)
		if(buffers[i] != NULL) delete (CBuffer*)buffers.item(i);

	for(int i = 0; i < sockets.count; i++)
		if (sockets[i] != NULL) delete (CSocket*)sockets.item(i);

	for(int i = 0; i < files.count; i ++)
		if (files[i] != NULL) CTools::BinClose((HANDLE)files.item(i));
}
...
{
	CSocket* sock1	= (CSocket*)sockets.item(sockid);
...
{
	CSocket* sock = (CSocket*)sockets.item(sockid);
	CBuffer* buff = (CBuffer*)buffers.item(buffid);
...
int receivemessage(int sockid, int len, int buffid)
{
	...
	if (sock == NULL) return -1;
	if (buff == NULL) return -2;
	int size = sock->receivemessage(len, buff);
	if (size < 0)
	{
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readchar());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readshort());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readushort());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readint());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readuint());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readfloat());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readdouble());
}
...
{
	CBuffer*buff = (CBuffer*)buffers.item(buffid);
	return ((buff == NULL) ? 0 : buff->readstring());
}
...
int getpos(int pos, int buffid)
{
	...
	return ((pos == 0) ? buff->writepos : buff->readpos);
}
...
{
	CBuffer*destination = (CBuffer*)buffers.item(destinationid);
	if(destination == NULL) return false;
	CBuffer*source = (CBuffer*)buffers.item(sourceid);
...
{
	HANDLE file = (HANDLE)files.item(fileid);
