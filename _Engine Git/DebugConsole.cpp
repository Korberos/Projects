...
#define DEBUG_CONSOLE_TOGGLE_KEY	sf::Keyboard::Tilde
...
void DebugConsole::Input( void )
{
	...
	if ( input_system->hasTextString() )
	{
			TypingString += input_system->Get_Text_String();
	}

	if (input_system->Get_Key(sf::Keyboard::BackSpace) == 1 && !TypingString.empty()) TypingString.erase(--TypingString.end());

	if ( input_system->Get_Key( sf::Keyboard::Return ) == 1 && !TypingString.empty() )
	{
...
void DebugConsole::Render( void )
{
	...
	{
		ConsoleFont->Render_Text( ConsoleStrings[ ConsoleStrings.size() - 1 - i ].c_str(), 4, ConsoleHeight - ( (4 + ConsoleFont->Get_Font_Height() ) * ( i + 2 ) ) );
	}

	ConsoleFont->Render_Text( TypingString.c_str(), 4, ConsoleHeight - (4 + ConsoleFont->Get_Font_Height() ) );
}
