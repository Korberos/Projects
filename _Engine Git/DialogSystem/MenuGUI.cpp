...
const RapidXML_Node* MenuGUI::Load_Menu_GUI_XML( const RapidXML_Doc* xml )
{
	const RapidXML_Node* mainNode = xml->first_node();
	FAIL_IF ( mainNode == NULL ) return NULL;

	for ( RapidXML_Node* childIter = mainNode->first_node(); childIter != NULL; childIter = childIter->next_sibling() )
	{
		std::string new_element_name( childIter->name() );
		...
		FAIL_IF ( (*element_iter).second->Valid_Element_XML( childIter ) == false ) { return NULL; }
	}
...
{
	const RapidXML_Doc* menu_xml = XMLWrapper::Get_Instance()->LoadXMLFile( file_name );
	FAIL_IF ( menu_xml == NULL ) return NULL;

	const RapidXML_Node* menu = MenuGUI::Load_Menu_GUI_XML( menu_xml );
	...
	for ( const RapidXML_Node* childIter = menu->first_node(); childIter != NULL; childIter = childIter->next_sibling() )
	{	
		MenuGUIElement* new_element = NULL;
		std::string new_element_name( childIter->name() );
		ElementTypeListIteratorType element_iter = ElementTypes.find( new_element_name );
		FAIL_IF ( element_iter == ElementTypes.end() && "The XML was not valid for loading a MenuGUI." ) { return NULL; }
		FAIL_IF ( ( new_element = (*element_iter).second->Create_Instance( childIter) ) == NULL && "An element failed to be created properly. MEMORY UNRESOLVED." ) { return NULL; }
