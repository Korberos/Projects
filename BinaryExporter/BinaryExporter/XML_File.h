...
public:
	...
	XML_Tag*				Parent;
	std::vector<XML_Tag*>	Children;
	std::vector<std::pair<std::string, std::string>> Elements;
	
	XML_Tag* Find_Child( char* string, unsigned int depth );
	std::string Find_Element( std::string );
};
...
std::string XML_Tag::Find_Element( std::string name )
{
...
XML_File* XML_File::Load_XML_File( const char* xml_file )
{
	...
	if ( !new_xml_file ) return NULL;

	if ( !new_xml_file->Load_XML_Data( xml_file ) )
	{
		delete new_xml_file;
		return NULL;
	}
...
bool XML_File::Load_XML_Data( const char* xml_file )
{
	...
	if ( file_stream.bad() || !file_stream.good() )
	{
		file_stream.close();
		return false;
	}
	...
	while ( !file_stream.eof() )
	{
		...
		if ( within_comment )
		{
			if ( !Iterate_Comment( line_string, col_index, within_comment ) ) return false;
			else
			{
				Iterate_Whitespace( line_string , col_index );
		...
		if ( line_string[col_index] != '<' ) return false;
		...
		while ( line_string[col_index] == '<' )
		{
			...
			{
			case '/':
				if ( !Remove_XML_Tag( line_string , col_index ) ) return false;
				next_line = true;
				break;

			case '?':
				if ( !Add_XML_Version_Info( line_string , col_index , xml_version) ) return false;
				next_line = true;
				break;

			case '!':
				if ( !Begin_Comment( line_string , col_index , within_comment ) ) return false;
				next_line = true;
				break;

			default:
				if (!Add_XML_Tag( line_string, col_index ) ) return false;
				next_line = false;
		...
		if ( !End_Of_XML_Line( line_string, col_index ) ) return false;
	}

	//  If we reach the end of the file and there is still a tag in the stack, then
	//  we know that the XML is not valid
	if ( !XML_Tag_Stack.empty() )
	{
		if (within_comment) return false; // Separate return just to notify any debuggers that this is the reason... a comment never ended
		return false;
	}
...
bool XML_File::Remove_XML_Tag( char* string, unsigned int& index )
{
	...
	if (!XML_Tag_Stack.top()->Is_Same_Tag(tag_name)) return false;
	else
	{
...
bool XML_File::Add_XML_Version_Info( char* string, unsigned int& index, char* version_string )
{
	...
	++index;
	if (strncmp(&string[index], "xml version=\"", 13) != 0) return false;
