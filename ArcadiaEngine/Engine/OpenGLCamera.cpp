...
void COpenGLFrame::Determine_Ray( int mouse_x, int mouse_y, int window_width, int window_height, float fov, float distance, Vector3<float>& start, Vector3<float>& end ) const
{
	...
	// Consume.
	float fov_angle_y = 60.0f;
	float size_x = float(1024);
	float size_y = float(768);

	// Recompute.
	float aspect_xy = ((float)size_x) / ((float)size_y);
	float tan_y = tanf(fov_angle_y / 2);
	float tan_x = tan_y * aspect_xy;
