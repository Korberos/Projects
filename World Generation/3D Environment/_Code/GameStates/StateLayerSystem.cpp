...
void StateLayerSystem::Initialize(HWND Handle, HINSTANCE Instance, int W_Width, int W_Height, char* KeyArray)
{
	...
	GameState* NewState = new GS_World;
	NewState->SetBitFlag(BIT_ALLBITS);
	LayersToAdd.push_back(NewState);
}
...
bool StateLayerSystem::MainProcess(void)
{
	...
	while (LayersToAdd.size())
	{
		...
		LayersToAdd[0]->Initialize(&m_fTimeSlice);
		LayersToAdd.erase(LayersToAdd.begin());
	}
	
	if (GameStateLayers.size() == 0)
	{
		return false;
	}
	...
	for (int i = (int)GameStateLayers.size() - 1; i >= 0; i -= 1)
		if (GameStateLayers[i]->BITFLAG & BIT_INPUT)
			if (!GameStateLayers[i]->Input(m_nMouseX, m_nMouseY, Key_Array)) return false;

	// Each gamestate layer will complete it's main process according to
	// its bitflags, and will add to the draw lists in the main game state
	for (int i = (int)GameStateLayers.size() - 1; i >= 0; i -= 1)
		if (GameStateLayers[i]->BITFLAG & BIT_UPDATE)
			if (!GameStateLayers[i]->Update(m_nMouseX, m_nMouseY)) return false;
